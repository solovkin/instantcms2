...
icms.datagrid = (function ($) {
    ...
    this.init = function(){
        ...
        if (this.options.is_pagination){
            $('.datagrid_resize select').change(function(event){
        ...
        if (this.options.is_selectable){
            $('#datagrid tr[class!=filter] td').live('click', function(event){
    ...
    this.loadRows = function (){
        ...
        $.post(this.options.url, {filter: filter_query}, this.rowsLoaded, 'json');
