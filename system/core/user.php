...
class cmsUser {
    ...
    public function __construct(){
        ...
        if ($this->id){
            ...
            $this->perms = self::sessionGet('user:perms');
            $this->is_admin = (bool)self::sessionGet('user:is_admin');
            $this->is_logged = true;
...
//============================================================================//
    ...
    public static function logout() {
        ...
        cmsEventsManager::hook('user_logout', $user);

        self::$instance->groups = array(GUEST_GROUP_ID);
        self::$instance->is_logged = false;
        self::$instance->id = 0;

        self::sessionUnset('user');
        self::unsetCookie('auth');

        self::deleteSession($user['id']);
        self::deleteOldSessions();
...
//============================================================================//
    ...
    public static function isLogged(){
        return self::isSessionSet('user');
    }

    public static function isAdmin() {
        return (self::isSessionSet('user')) ? (bool)self::sessionGet('user:is_admin') : false;
    }
...
//============================================================================//
    ...
    public static function isPermittedLimitReached($subject, $permission, $current_value=0, $default=true){

        $user = self::getInstance();

        if ($user->is_admin){ return $default; }

        if (!isset($user->perms[$subject])) { return $default; }
        if (!isset($user->perms[$subject][$permission])) { return $default; }
        if ($user->perms[$subject][$permission] >= (int)$current_value) { return true; }
