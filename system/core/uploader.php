...
//============================================================================//
    ...
    public function resizeImage($source_file, $size){
        ...
        $dir_num    = sprintf('%03d', intval($user->files_count/100));
        $dest_dir   = $cfg->upload_path . "u{$user->id}/{$dir_num}";

        @mkdir($dest_dir, 0777, true);

        $dest_info  = pathinfo($source_file);
        $dest_ext   = $dest_info['extension'];
        $dest_file  = substr(md5( $user->id . $user->files_count . time() . $size['width'] ), 0, 8) . '.' . $dest_ext;
...
//============================================================================//
     ...
     * @return array
     */
    public function upload($post_filename, $allowed_ext = false, $destination = false){

        if (isset($_GET['qqfile'])){
            return $this->uploadXHR($post_filename, $allowed_ext, $destination);
        }

        if (isset($_FILES[$post_filename])){
            return $this->uploadForm($post_filename, $allowed_ext, $destination);
        }
...
//============================================================================//
     ...
     * @return array
     */
    public function uploadForm($post_filename, $allowed_ext = false, $destination = false){
        ...
        $dest_name  = basename($_FILES[$post_filename]['name']);
        ...
        if (!$destination){
            ...
            $dest_file = substr(md5( $user->id . $user->files_count . time() ), 0, 8) . '.' . $dest_ext;
...
//============================================================================//
     ...
     * @return array
     */
    public function uploadXHR($post_filename, $allowed_ext = false, $destination = false){
